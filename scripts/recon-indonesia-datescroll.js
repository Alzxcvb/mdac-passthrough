// Test scrolling the arrival-date virtuoso to reach "01 JULY 2026".
const { chromium } = require("playwright");
const ORIGIN = "https://allindonesia.imigrasi.go.id";
const clickPT = (page, label) => page.evaluate((l) => {
  ...
  if (!n) return false; let c = n; for (let i = 0; i < 10 && c; i++, c = c.parentElement) if (getComputedStyle(c).cursor === "pointer") { c.click(); return true; } n.click(); return true;
}, label);
const openV = (page, sel) => page.evaluate((s) => {
  const inp = document.querySelector(s); if (!inp) return false; let n = inp, target = inp;
  for (let i = 0; i < 6 && n; i++, n = n.parentElement) if (getComputedStyle(n).cursor === "pointer") { target = n; break; }
  const r = target.getBoundingClientRect(); const o = { bubbles: true, cancelable: true, clientX: r.left + r.width / 2, clientY: r.top + r.height / 2, view: window };
  target.dispatchEvent(new MouseEvent("mousedown", o)); target.dispatchEvent(new MouseEvent("mouseup", o)); target.dispatchEvent(new MouseEvent("click", o)); return true;
}, sel);
async function pickV(page, sel, text) {
  await openV(page, sel); await page.waitForTimeout(500);
  const s = page.locator('input[placeholder="Search" i]').first();
  if (await s.isVisible().catch(() => false)) { await s.fill(text); await page.waitForTimeout(700); }
  for (let p = 0; p < 60; p++) { const ok = await page.evaluate((t) => { const sc = document.querySelector('[data-virtuoso-scroller="true"]'); if (!sc) return false; for (const r of [...sc.querySelectorAll("[data-index]")]) if ((r.textContent || "").trim() === t) { ((r.firstElementChild) || r).click(); return true; } sc.scrollTop += sc.clientHeight * 0.7; return false; }, text); if (ok) return true; await page.waitForTimeout(120); } return false;
}
(async () => {
  ...
  await openV(page, "#std_arrival_date_foreigner_individual"); await page.waitForTimeout(800);
  // manual scroll probe — log scrollTop progression + visible rows
  for (let p = 0; p < 40; p++) {
    const snap = await page.evaluate(() => {
      const sc = document.querySelector('[data-virtuoso-scroller="true"]');
      if (!sc) return null;
      const rows = [...sc.querySelectorAll("[data-index]")].map(r => (r.textContent||"").trim());
      const before = sc.scrollTop;
      sc.scrollTop += sc.clientHeight * 0.7;
      return { before, after: sc.scrollTop, clientH: sc.clientHeight, scrollH: sc.scrollHeight, first: rows[0], last: rows[rows.length-1], n: rows.length, hasJul1: rows.includes("01 JULY 2026") };
    });
    if (!snap) { console.log("no scroller"); break; }
    console.log(`p${p}: top ${snap.before}->${snap.after} clientH=${snap.clientH} scrollH=${snap.scrollH} rows=${snap.n} [${snap.first} .. ${snap.last}] hasJul1=${snap.hasJul1}`);
    if (snap.hasJul1) break;
